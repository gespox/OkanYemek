...
<input type="text" name="tatli_adi" placeholder="Bakmak İstediğiniz Tatlının Adı" />
...
<?php 
        ...
        if($_POST){			
            $sec        =    mysqli_query($baglanti,"select * from tatlilar where tatli_adi='".$_POST['tatli_adi']."'" );
		while($gelen=mysqli_fetch_array($sec)){
			...
			echo "<tr>		
					...
					Tatlı Açıklaması:
                    <td>$tatli_aciklamasi</td><br/>
					Tatlı Fiyatı:
					<td>$tatli_fiyati</td><br/>
                </tr>";
