...
<input type="text" name="icecek_adi" placeholder="Bakmak İstediğiniz İçecek Adı" />
...
<?php 
        ...
        if($_POST){			
            $sec        =    mysqli_query($baglanti,"select * from icecek where icecek_adi='".$_POST['icecek_adi']."'" );
		while($gelen=mysqli_fetch_array($sec)){
			...
			echo "<tr>		
					...
					İçecek Açıklaması:
                    <td>$icecek_aciklamasi</td><br/>
					İçecek Fiyatı:
					<td>$icecek_fiyati</td><br/>
                </tr>";
